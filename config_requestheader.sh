...
function create_ca() {
    ...
    openssl req -x509 -new -nodes -key "${DIR}/rootCA.key" -sha512 -days 3655 -out "${DIR}/rootCA.crt" -subj "/C=CZ/ST=Moravia/O=My Private Org Ltd./CN=Test CA" -extensions v3_ca -config "${DIR}/custom.cnf" -passin "pass:${PASSWORD}"
}
...
function create_client() {
    ...
    openssl req -new -sha256 -key "${DIR}/client.key" -subj "/C=CZ/ST=Moravia/O=My Private Org Ltd./CN=somewhere.com" -out "${DIR}/client.csr"
    openssl x509 -req -in "${DIR}/client.csr" -CA "${DIR}/rootCA.crt" -CAkey "${DIR}/rootCA.key" -CAcreateserial -out "${DIR}/client.crt" -days 1024 -sha256 -extfile "${DIR}/custom.cnf" -extensions client_auth -passin "pass:${PASSWORD}"
}
...
function create_server() {
    ...
    openssl req -new -sha256 -key "${DIR}/server.key" -subj "/CN=${server_name}" -out "${DIR}/server.csr"
    openssl x509 -req -in "${DIR}/server.csr" -CA "${DIR}/rootCA.crt" -CAkey "${DIR}/rootCA.key" -CAcreateserial -out "${DIR}/server.crt" -days 1024 -sha256 -extfile "${DIR}/custom.cnf" -extensions server_auth -passin "pass:${PASSWORD}"
}
